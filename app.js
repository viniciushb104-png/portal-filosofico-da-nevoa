...
function finishRound(){const g=games[currentGame];$('#quiz').style.display='none';$('#scorePanel').style.display='block';const old=state.completed[currentGame]||0;let gained=0;const wasDaily=dailyPending;if(roundScore>old){gained+=roundScore-old;state.xp+=roundScore-old;state.completed[currentGame]=roundScore;}if(dailyPending){gained+=15;state.xp+=15;state.daily=new Date().toISOString().slice(0,10);dailyPending=false;}saveState();if(gained)toast(`+${gained} XP conquistados`);$('#badgeBig').textContent=roundScore===30?'🏆':roundScore>=20?'🏮':'🕯️';$('#finalTitle').textContent=roundScore>=20?'Porta aberta!':'A porta range...';$('#finalText').textContent=`Você conquistou ${roundScore} de 30 XP em ${g.title}. ${roundScore>=20?'A mansão reconhece sua investigação.':'Você pode retornar e tentar novamente.'}`;$('#badgeText').textContent=roundScore===30?'Desafio perfeito':'Desafio concluído';checkAchievements();updateUI();if(window.NevoaOnline?.getCode()){window.NevoaOnline.claimProgress(currentGame,roundScore).then(applyOnlineClaim).catch(()=>{});if(wasDaily)window.NevoaOnline.claimDaily().then(applyOnlineClaim).catch(()=>{});}}
...
const achievementDefs=[
 ['first','Primeira Porta','🚪','Conclua qualquer sala.'],['perfect','Olhar de Coruja','🦉','Faça 30/30 em uma sala.'],['logic','Caçador de Falácias','🧠','Conclua a Sala da Lógica.'],['ethics','Juiz das Sombras','⚖️','Conclua o Tribunal das Sombras.'],['knowledge','Guardião do Espelho','🪞','Conclua Descartes e Platão.'],['explorer','Explorador da Mansão','🗺️','Conclua quatro salas.'],['socrates','Discípulo do Diálogo','🏛️','Conclua a Mansão de Sócrates.'],['platform','Caminhante de Atenas','🎮','Conclua a Fase 1 do Labirinto dos Filósofos.'],['platoCave','Libertado da Caverna','🌞','Conclua A Caverna de Platão.'],['rpgParadoxia','Cidadão de Paradoxia','🎭','Conclua o Capítulo I do RPG Paradoxia.'],['guardian','Guardião do Portal','🦇','Alcance 360 XP.']
];
function checkAchievements(){const vals=Object.values(state.completed);if(vals.length)state.achievements.first=true;if(vals.some(v=>v===30))state.achievements.perfect=true;if(state.completed.logica>=20)state.achievements.logic=true;if(state.completed.etica>=20)state.achievements.ethics=true;if(state.completed.descartes>=20&&state.completed.platao>=20)state.achievements.knowledge=true;if(vals.filter(v=>v>=20).length>=4)state.achievements.explorer=true;if(state.socrates>=7)state.achievements.socrates=true;if(state.completed.plataforma>=60)state.achievements.platform=true;if(state.completed.plataoPlataforma>=70)state.achievements.platoCave=true;if(state.completed.rpgParadoxia>=120)state.achievements.rpgParadoxia=true;if(state.xp>=360)state.achievements.guardian=true;saveState();}
